...
public class Wobbling : MonoBehaviour {    
    ...
    private void Update() {
        ...
        transform.position = new Vector3(offset.x / 2, offset.y * 1.05f, offset.z / 2) + pivot.position; //offset being default //offset can /2 or /4 or *2 for empty space
 
        // Orient so "up" points away from the pivot,
        // and "forward" aligns roughly to the pivot's forward.
        transform.rotation = Quaternion.LookRotation(new Vector3(offset.x, offset.y * 2f, offset.z),  -pivot.forward) //offset being default //mb offset.z/2?
